...
VkBool32 debug_callback(
	...
	if (pCallbackData->messageIdNumber == 0x6f7a814b)
		return false;
	// SYNC-HAZARD-PRESENT-AFTER-WRITE
	// I'm pretty sure I'm doing this properly
	if (pCallbackData->messageIdNumber == 0xe17ab4ae)
		return false;
	...
	Log::log(Log::Msg{
		.severity = severity,
		.type = "Vulkan " +
			to_string(static_cast<vk::DebugUtilsMessageTypeFlagBitsEXT>(
				messageTypes)) +
			" " +
			to_string(static_cast<vk::DebugUtilsMessageSeverityFlagBitsEXT>(
				messageSeverity)) +
...
const std::vector<vk::Format> valid_formats = {
	vk::Format::eR8G8B8Srgb,         vk::Format::eB8G8R8Srgb,
	vk::Format::eR8G8B8A8Srgb,       vk::Format::eB8G8R8A8Srgb,
	vk::Format::eA8B8G8R8SrgbPack32, vk::Format::eR16G16B16Sfloat,
	vk::Format::eR16G16B16A16Sfloat, vk::Format::eR32G32B32Sfloat,
	vk::Format::eR32G32B32A32Sfloat, vk::Format::eR64G64B64Sfloat,
	vk::Format::eR64G64B64A64Sfloat, vk::Format::eB10G11R11UfloatPack32,
};
...
Render::Render(
...

	{
		...
		vk::ApplicationInfo app_info(
			"Guidestone", 0, "Guidestone", 0, VK_API_VERSION_1_3);
		vk::InstanceCreateInfo instance_create(
			{}, &app_info, {}, vulkan_extensions);
				...
				supported_formats.erase(
					std::remove_if(
						...
						[](vk::SurfaceFormatKHR format) {
							bool valid_space = format.colorSpace ==
								vk::ColorSpaceKHR::eVkColorspaceSrgbNonlinear;
							bool valid_format =
								std::find(
									valid_formats.begin(), valid_formats.end(),
									format.format) != valid_formats.end();
	...
	}
	{
		...
		vk::StructureChain<
			vk::DeviceCreateInfo, vk::PhysicalDeviceVulkan13Features>
			device_info(
				vk::DeviceCreateInfo({}, queue_create, {}, device_ext),
				vk::PhysicalDeviceVulkan13Features());
		device_info.get<vk::PhysicalDeviceVulkan13Features>()
	...
	for (int i = 0; i < frame_concurrency; i++) {
		...
		f.command_pool = device.createCommandPool(
			vk::CommandPoolCreateInfo({}, device_config.queue_family));
		f.command_buffer =
			device
				.allocateCommandBuffers(vk::CommandBufferAllocateInfo(
					f.command_pool, vk::CommandBufferLevel::ePrimary, 1))
				.front();
		f.submission_fence = device.createFence(
...
void Render::reconfigureSwapchain() {
	...
	vk::SurfaceCapabilitiesKHR caps =
		device_config.physical_device.getSurfaceCapabilitiesKHR(surface);
	if (caps.currentExtent != vk::Extent2D(0xFFFFFFFF, 0xFFFFFFFF) &&
		caps.currentExtent != vk::Extent2D(0, 0)) {
...

	{
		...
		swapchain = device.createSwapchainKHR(vk::SwapchainCreateInfoKHR(
			{}, surface, image_count,
			device_config.surface_colour_format.format,
			device_config.surface_colour_format.colorSpace, surface_extent, 1,
...
uint32_t Render::acquireImage(vk::Semaphore semaphore) {
	...
	auto image_index_result =
		device.acquireNextImageKHR(swapchain, UINT64_MAX, semaphore, nullptr);
...
void Render::renderFrame() {
	...
	auto fence_result =
		device.waitForFences(frame.submission_fence, false, UINT64_MAX);
...

	{
...

		{
			vk::ImageMemoryBarrier2 image_barrier(
				...
				device_config.queue_family, device_config.queue_family,
				images[image_index],
				vk::ImageSubresourceRange(
					vk::ImageAspectFlagBits::eColor, 0, 1, 0, 1));
			frame.command_buffer.pipelineBarrier2(
				vk::DependencyInfo({}, {}, {}, image_barrier));
		}

		{
			vk::Viewport viewport(
				0, 0, surface_extent.width, surface_extent.height, 0, 1);
			...
			vk::ClearValue clear_value(vk::ClearColorValue(
				std::array<float, 4>({{0.0, 0.3, 0.8, 1.0}})));
			vk::RenderingAttachmentInfo colour_attachment(
				image_views[image_index],
				vk::ImageLayout::eColorAttachmentOptimal);
			colour_attachment.setLoadOp(vk::AttachmentLoadOp::eClear)
				.setStoreOp(vk::AttachmentStoreOp::eStore)
				.setClearValue(clear_value);
			vk::RenderingInfo rendering_info(
				{}, scissors, 1, 0, colour_attachment);
...

		{
			vk::ImageMemoryBarrier2 image_barrier(
				...
				vk::PipelineStageFlagBits2::eColorAttachmentOutput, {},
				vk::ImageLayout::eColorAttachmentOptimal,
				vk::ImageLayout::ePresentSrcKHR, device_config.queue_family,
	...
	vk::SemaphoreSubmitInfo rendering_semaphore_info(
		frame.rendering_semaphore, {},
		vk::PipelineStageFlagBits2::eColorAttachmentOutput);
	vk::SubmitInfo2 submit_info(
		{}, acquire_semaphore_info, cmd_info, rendering_semaphore_info);
	queue.submit2(submit_info, frame.submission_fence);

	vk::Result result = queue.presentKHR(
		vk::PresentInfoKHR(frame.rendering_semaphore, swapchain, image_index));
